...
//  BSNShareCenter
//
//  Created by Jun Xia on 15/6/5.
//
//

#import "BSNSCWeChat.h"
#ifdef ShareCenter
#import "WXApi.h"
#else
...
- (void)shareWithShareModel:(BSNSCShareModel *)shareModel completeBlock:(void(^)(BOOL success, NSError *error))completeBlock {
    ...
    if (![WXApi isWXAppInstalled]) {
        NSError *error = [BSNSCError errorWithCode:BSNSCShareError message:BSNSCWeChatNoInstallMsg];
        BLOCK_SAFE_RUN(completeBlock, NO, error);
        return;
    }
